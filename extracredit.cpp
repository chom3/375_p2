...
float closestPair(Point Px[], Point Py[], int n)
{
    ...
    //calculate the smallest distance dl on left of middle point and dr on right and depth
...
int main(int argc, char* argv[]){
    ...
    writeFile << "The first index is: " << pts[a].index << " (" << pts[a].x << "," << pts[a].y << "," << pts[a].z << ")" << " (Line: " << a+1 << " in the input file)"<< endl;
    writeFile << "The second index is: " << pts[b].index << " (" << pts[b].x << "," << pts[b].y << "," << pts[b].z<<  ")" <<" (Line: " << b+1 << " in the input file)"<< endl;
